...
function generatePaymentEmail(leadName) {
  return {
    subject: `تفعيل حسابك مدى الحياة في Hisabi 🇲🇦`,
    body: `أهلاً ${leadName}،

سعيد جداً باهتمامك بالاشتراك في Hisabi لتسهيل إدارة أعمالك وحساباتك.

بمناسبة الإطلاق، نقدم لك عرضاً خاصاً ومحدوداً: تفعيل الحساب **مدى الحياة (Lifetime Access) بقيمة 299 درهم فقط** (دفع مرة واحدة فقط، والاستخدام مدى الحياة بدون أي اشتراكات شهرية أو مصاريف إضافية).

لتفعيل حسابك مباشرة، يمكنك إجراء تحويل بنكي (Virement) بقيمة 299 DH إلى حسابنا في CIH Bank:
...
بعد إجراء التحويل، يرجى الرد على هذا البريد وإرفاق صورة أو لقطة شاشة لوصل التحويل (Reçu de virement)، وسنقوم بتفعيل حسابك مدى الحياة فوراً وإرسال بيانات الدخول الخاصة بك.
